...
public final class MultidimensionalKernelSegmenter {
    ...
    private final OverlapDetector<CopyRatio> copyRatioOverlapMidpointDetector;
    ...
    public MultidimensionalKernelSegmenter(final CopyRatioCollection denoisedCopyRatios,
        ...
        copyRatioOverlapMidpointDetector = denoisedCopyRatios.getOverlapDetector();
        ...
        multidimensionalPointsPerChromosome = denoisedCopyRatios.getRecords().stream()
                .map(cr -> new MultidimensionalPoint(
                        ...
                        allelicCountOverlapDetector.getOverlaps(cr).stream()
                                .min(LocatableCollection.LEXICOGRAPHICAL_ORDER_COMPARATOR::compare)
    ...
    public CRAFSegmentCollection findSegmentation(final int maxNumChangepointsPerChromosome,
        ...
        final List<CRAFSegment> segments = new ArrayList<>();
        for (final String chromosome : multidimensionalPointsPerChromosome.keySet()) {
            ...
            if (numMultidimensionalPointsInChromosome < MIN_NUM_POINTS_REQUIRED_PER_CHROMOSOME) {
                ...
                segments.add(constructCRAFSegment(new SimpleInterval(chromosome, start, end)));
            }
        ...
        return new CRAFSegmentCollection(allelicCounts.getSampleName(), segments);
    }
    ...
    private CRAFSegment constructCRAFSegment(final SimpleInterval interval) {
        return new CRAFSegment(
                interval,
                copyRatioOverlapMidpointDetector.getOverlaps(interval).stream()
